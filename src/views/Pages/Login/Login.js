...
class Login extends Component {
  ...
  async login(){
    ...
    if(email === "" || password === ""){
      this.setState({
        isLogin: false,
        errorLogin: 'Email hoặc password chưa đúng!'
      })
    }
    else{
      await this.props.Login(this.state.email,this.state.password);
      const token = localStorage.getItem('token');
      if(this.props.isAuthentication === true || token !== null){
        await this.props.history.push('/planning-year-list');
      }
      else{
        this.setState({
          isLogin: false,
              ...
              <CardGroup>
                <Card className="p-4">
                  <CardBody>
                    <Form>
                      ...
                      { (!this.state.isLogin) ?
                        <Row>
                            <Col xs="12" md="12" marginHeight="-30" >
                              <Alert color="danger">
                ...
                <Card className="text-white bg-primary py-5 d-md-down-none" style={{ width: '44%' }}>
                  <CardBody className="text-center">
                    <div>
                      <h2>Sign up</h2>
                      <p>DMS Schedule Task, Daily meeying schedule</p>
                      <Link to="/register">
